...
export async function getPics(request: FastifyRequest, reply: FastifyReply) {
    try {
        ...
        if (existingFiles.length > 0) {
            const modifiedFiles = existingFiles.map(
                (file) => '/uploads/' + file
            );
...
export async function checkMicroservicesHook(
    ...
    try {
        if (
            ...
            Server.microservices.get(process.env.PROFIL_ADDR || 'profil') ===
                false
        ) {
            const errorMessage = createErrorResponse(
                503,
            ...
            Server.microservices.get(process.env.FRIENDS_ADDR || 'friends') ===
                false
        ) {
            const errorMessage = createErrorResponse(
                503,
...
export async function checkMicroservices() {
    try {
        const [profilStatus, authStatus, gameStatus, friendsStatus] =
            await Promise.all([
                checkService(
                    process.env.PROFIL_ADDR || 'localhost',
                    process.env.PROFIL_PORT || '8081'
                ),
                checkService(
                    process.env.AUTH_ADDR || 'localhost',
                    process.env.AUTH_PORT || '8082'
                ),
                checkService(
                    process.env.GAME_ADDR || 'localhost',
                    process.env.GAME_PORT || '8083'
                ),
                checkService(
                    process.env.FRIENDS_ADDR || 'localhost',
                    process.env.FRIENDS_PORT || '8084'
                ),
            ]);

        Server.microservices.set(
            process.env.PROFIL_ADDR || 'profil',
            profilStatus
        );
        ...
        Server.microservices.set(
            process.env.FRIENDS_ADDR || 'friends',
            friendsStatus
        );
    } catch (err) {

...
import {
    jwtPluginHook,
    jwtPluginRegister,
} from './shared/plugins/jwtPlugin.js';
import {
    checkMicroservices,
            ...
            await server.register(fastifySwagger, {
                openapi: {
                    info: {
                        title: 'Game Service API',
                        description:
                            'API documentation for the Game microservice',
                        version: '1.0.0',
                    },
                    servers: [
                        {
                    ...
                    tags: [
                        ...
                        },
                        {
                            name: 'friends',
                            description:
                                'Endpoints for managing friends and connections',
                        },
                        {
                            name: 'matches',
                ...
                (err, address) => {
                    if (err) {
                        ...
                        if (err.message.includes('EADDRINUSE'))
                            server.log.error(
                                `Port ${Number(process.env.API_PORT) || 8080} is already in use`
                            );
                        process.exit(1);
                    }

...
const friendsUrl = 'http://localhost:8080/api/v1/friends';
let token1 = '';
let userId1 = '';
const user1 = {
    username: 'test',
...
try {
    ...
    //Check relation false
    {
        ...
        if (response.status === 500) {
            ...
            if (!responseData.status) {
                ...
                countFailed += 1;
                console.log(`   ${UNDERLINE}${name}${RESET} (${BOLD}${method}${RESET})(${BOLD}${path}${RESET}): ${RED}failed (Code: ${response.status}) expected false ❌${RESET}`);
            }
        }
    }
    // ----------------------------------------------------------------
    ...
    //Delete all relations user1 failed
    {
        ...
        if (response.status === 500) {
            ...
            countFailed += 1;
            console.log(`   ${UNDERLINE}${name}${RESET} (${BOLD}${method}${RESET})(${BOLD}${path}${RESET}): ${RED}failed (Code: ${response.status}) expected 404 NOT_FOUND❌${RESET}`);
        }
        else {
            console.log(`   ${UNDERLINE}${name}${RESET} (${BOLD}${method}${RESET})(${BOLD}${path}${RESET}): ${GREEN}success (Code: ${response.status}) ✅${RESET}`);
        }

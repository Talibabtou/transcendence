...
export async function initDb(): Promise<Database<sqlite3.Database, sqlite3.Statement>> {
  try {
    ...
    if (!computer) {
      await db.run(
        'INSERT INTO users (id, role, username, password, email, created_at) VALUES ("b36c8474-a8a6-b889-af67-08ccca5a7593" ,"admin", "computer", "computer", "computer@computer.com", CURRENT_TIMESTAMP);'
      );
      let gameState = false;

...
export async function checkRevoked(request, reply) {
    try {
...
export async function twofaGenerate(request, reply) {
    try {
        const id = request.params.id;
        const { two_factor_enabled } = await request.server.db.get('SELECT two_factor_enabled FROM users WHERE id = ?;', [id]);
        ...
        const qrCodeImage = await qrcode.toDataURL(secretCode.otpauth_url);
        const qrCodeReponse = {
            qrcode: qrCodeImage,
...
export async function twofaValidate(request, reply) {
    try {
        ...
        const data = await request.server.db.get('SELECT two_factor_secret, two_factor_enabled FROM users WHERE id = ?;', [id]);
        ...
        if (verify) {
            await request.server.db.run('UPDATE users SET verified = true, two_factor_enabled = true WHERE id = ?', [id]);
            return reply.code(200).send();
        }
        const errorMessage = createErrorResponse(401, ErrorCodes.UNAUTHORIZED);
        return reply.code(401).send(errorMessage);
...
export async function twofaDisable(request, reply) {
    try {
        ...
        await request.server.db.run('UPDATE users SET two_factor_enabled = false, two_factor_secret = null WHERE id = ?', [id]);
        return reply.code(200).send();
    }
    catch (err) {
        request.server.log.error(err);

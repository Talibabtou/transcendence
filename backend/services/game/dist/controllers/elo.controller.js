...
async function calculateEloChange(winnerElo, loserElo) {
    const K_FACTOR = 32;
...
export async function getElo(request, reply) {
    ...
    try {
        ...
        const elo = (await request.server.db.get('SELECT * FROM elo INDEXED BY idx_elo_player_created_at WHERE player = ? ORDER BY created_at DESC LIMIT 1', [player_id]));
...
export async function getElos(request, reply) {
    ...
    try {
        ...
        const startTime = performance.now(); // Start timer
        const elos = (await request.server.db.all(query, ...params));
        recordMediumDatabaseMetrics('SELECT', 'elo', performance.now() - startTime); // Record metric
...
export async function dailyElo(request, reply) {
    const { player } = request.params;
    try {
        const startTime = performance.now(); // Start timer
        const dailyElo = (await request.server.db.get('SELECT player, match_date, elo FROM player_daily_elo WHERE player = ?', [player]));
...
export async function updateEloRatings(db, winner, loser) {
    // Get current ELO ratings
    let startTime = performance.now(); // Start timer
    const winnerElo = (await db.get('SELECT * FROM elo INDEXED BY idx_elo_player_created_at WHERE player = ? ORDER BY created_at DESC LIMIT 1', [winner]));
    const loserElo = (await db.get('SELECT * FROM elo INDEXED BY idx_elo_player_created_at WHERE player = ? ORDER BY created_at DESC LIMIT 1', [loser]));
...
export async function getLeaderboard(request, reply) {
    ...
    try {
      ...
      SELECT 
        ...
        COALESCE(pms.total_matches - pms.active_matches - pms.victories, 0) as defeats

...
import { recordFastDatabaseMetrics, recordMediumDatabaseMetrics, recordSlowDatabaseMetrics, matchCreationCounter, matchTournamentCounter, } from '../telemetry/metrics.js';
...
export async function createMatch(request, reply) {
    ...
    try {
        ...
        if (prevMatches && prevMatches.length > 0) {
            for (const match of prevMatches) {
                ...
                if (match.duration_seconds > 600) {
                    ...
                    await request.server.db.run(`UPDATE matches SET active = FALSE, duration = NULL WHERE id = ?`, match.id);
        ...
        // Create new match
        startTime = performance.now(); // Start timer
        const newMatch = (await request.server.db.get('INSERT INTO matches (player_1, player_2, tournament_id) VALUES (?, ?, ?) RETURNING *', [player_1, player_2, tournament_id || null]));
        // Record metrics
...
export async function matchSummary(request, reply) {
    ...
    try {
        ...
        const startTime = performance.now();
        const matchSummaryResult = await request.server.db.get('SELECT total_matches, elo, active_matches, victories, win_ratio FROM player_match_summary WHERE player_id = ?', [id]);
...
export async function matchStats(request, reply) {
    ...
    try {
        ...
        let startTime = performance.now();
        const dailyPerformance = (await request.server.db.all('SELECT match_date, matches_played, wins, losses, daily_win_ratio FROM player_daily_performance WHERE player_id = ? ORDER BY match_date', [id])) || [];
        recordSlowDatabaseMetrics('SELECT', 'player_daily_performance', performance.now() - startTime);
        // Get goal durations for heatmap
        startTime = performance.now();
        const goalDurationsResult = await request.server.db.all('SELECT duration FROM player_goal_durations WHERE player = ?', [id]);
        ...
        startTime = performance.now();
        const matchDurationsResult = await request.server.db.all('SELECT match_duration FROM player_match_durations WHERE player_id = ?', [id]);
        ...
        startTime = performance.now();
        const eloRatingsResult = await request.server.db.all('SELECT elo FROM elo WHERE player = ? ORDER BY created_at', [id]);

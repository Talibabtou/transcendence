...
export async function createGoal(request, reply) {
    ...
    try {
        ...
        const newGoal = (await request.server.db.get('INSERT INTO goal (match_id, player, duration) VALUES (?, ?, ?) RETURNING *', match_id, player, duration || null));
...
export async function longestGoal(request, reply) {
    const { player_id } = request.params;
    try {
        const startTime = performance.now(); // Start timer
        const goal = (await request.server.db.get('SELECT duration FROM goal WHERE player = ? ORDER BY duration DESC LIMIT 1', [player_id]));
        recordMediumDatabaseMetrics('SELECT', 'goal', performance.now() - startTime); // Record metric
        if (!goal) {
            const errorResponse = createErrorResponse(404, ErrorCodes.GOAL_NOT_FOUND);
            return reply.code(404).send(errorResponse);
...
async function finalGoal(match, player, db) {
    try {
        ...
        const matchDurationResult = await db.get(`SELECT CAST((julianday('now') - julianday(created_at)) * 24 * 60 * 60 AS INTEGER) as duration_seconds 
       FROM matches WHERE id = ?`, [match.id]);
        recordFastDatabaseMetrics('SELECT', 'matches', performance.now() - startTime); // Record metric
        // Update match
        startTime = performance.now(); // Start timer
        await db.run(`UPDATE matches SET active = false, duration = ? WHERE id = ?`, matchDurationResult.duration_seconds, match.id);

...
export async function createGoal(
    ...
    try {
        ...
        recordFastDatabaseMetrics(
            'SELECT',
            'matches',
            performance.now() - selectStartTime
        ); // Record SELECT
        ...
        if (maxGoals && maxGoals.max_goals == 3) {
            console.log(
                `Max goals reached (${maxGoals.max_goals}), triggering final goal`
            );
            try {
                ...
                if (
                    ...
                    error.message.startsWith(ErrorCodes.ELO_NOT_FOUND)
                ) {
                    const errorResponse = createErrorResponse(
                        404,
                        ErrorCodes.ELO_NOT_FOUND
                    );
                    return reply.code(404).send(errorResponse);
                }
...
async function finalGoal(
    ...
    try {
       ...
       FROM matches WHERE id = ?`,
        ...
        const winner =
            match.player_1 === player ? match.player_1 : match.player_2;
        const loser =
            match.player_1 === player ? match.player_2 : match.player_1;

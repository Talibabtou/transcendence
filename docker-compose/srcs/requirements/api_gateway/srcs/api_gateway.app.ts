...
import { authSchema, Auth } from './modules/api_gateway.schema';
...
const server: fastify.FastifyInstance<Http2SecureServer, Http2ServerRequest, Http2ServerResponse>
	= fastify.fastify({
		...
		http2: true,
		https: {
				key: fs.readFileSync(env.API_KEY),
				cert: fs.readFileSync(env.API_CERTIF)
			}
	});
...
server.post('/auth/*', async (request: any, reply: any) => { 
	try {
		...
		const serviceUrl = `${env.AUTH_ADRR}${subpath}`;
...
server.post('/game/*', async (request: any, reply: any) => { 
	try {
		...
		const serviceUrl = `${env.GAME_ADRR}${subpath}`;
		const response = await fetch(serviceUrl, {
			method: 'POST',
			headers: {
			  'Content-Type': 'application/json',
...
const start = async () => {
	try {
		server.listen({ port: env.API_PORT, host: env.API_HOTE }, (err, address) => {
			if (err)

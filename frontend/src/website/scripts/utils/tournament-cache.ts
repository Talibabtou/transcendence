...
class TournamentCacheSingleton {
	 ...
	 */
	public registerPlayers(playerIds: string[], playerNames: string[], playerColors: string[]): string | null {
		try {
  ...
  // =========================================
	...
	public updateMatchFromServer(serverMatches: any[]): void {
		try {
			...
			const nextMatchIndex = this.findNextMatchIndex();
			if (nextMatchIndex >= 0) {
				this.setCurrentMatchIndex(nextMatchIndex);
			}

...
class TournamentCacheSingleton {
	...
	public static getInstance(): TournamentCacheSingleton {
		if (!TournamentCacheSingleton.instance) TournamentCacheSingleton.instance = new TournamentCacheSingleton();
	...
	public registerPlayers(playerIds: string[], playerNames: string[], playerColors: string[]): string {
		if (playerIds.length !== 4 || playerNames.length !== 4 || playerColors.length !== 4) throw new Error('Tournament requires exactly 4 players');
	...
	public initTournamentSchedule(): void {
		if (this.tournamentPlayers.length !== 4) throw new Error('Cannot initialize tournament without 4 players');
	...
	public startTournament(): void {
		...
		if (this.tournamentMatches.length > 0) this.tournamentMatches[0].isCurrent = true;
		this.saveToLocalStorage();
	}
	...
	public getCurrentMatch(): TournamentMatch | null {
		...
		if (this.currentMatchIndex >= this.tournamentMatches.length) return null;
	...
	public findNextMatchIndex(): number {
		for (let i = 0; i < this.tournamentMatches.length; i++) {
			if (!this.tournamentMatches[i].completed) return i;
		}
		return -1;
	}
	...
	public completeCurrentMatch(): void {
		...
		if (this.tournamentPhase === 'pool') this.currentMatchIndex++;
		else if (this.tournamentPhase === 'finals') this.tournamentPhase = 'complete';
		this.saveToLocalStorage();
	}
	...
	public areAllPoolMatchesCompleted(): boolean {
		...
		for (let i = 0; i < this.tournamentMatches.length - 1; i++) {
			if (!this.tournamentMatches[i].completed) return false;
		}
		return true;
	}
	...
	public setTournamentPhase(phase: TournamentPhase): void {
		...
		if (phase === 'pool' && this.tournamentMatches.length > 0) this.setCurrentMatchIndex(0);
		if (phase === 'finals' && this.tournamentMatches.length > 0) this.setCurrentMatchIndex(this.tournamentMatches.length - 1);
		this.saveToLocalStorage();
	}
	...
	public updateMatchFromServer(serverMatches: any[]): void {
		if (!serverMatches || !Array.isArray(serverMatches) || serverMatches.length === 0) return;

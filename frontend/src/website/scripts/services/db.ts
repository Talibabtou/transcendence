...
export class DbService {
	...
	private static async fetchApi<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
		...
		if (options.body instanceof FormData) {
			...
			if (token) headersForFormData['Authorization'] = `Bearer ${token}`;
			if (options.headers) {
				for (const [key, value] of Object.entries(options.headers)) {
					if (key.toLowerCase() !== 'content-type') headersForFormData[key] = value as string;
				}
			}
	...
	private static async handleApiResponse<T>(response: Response): Promise<T> {
		...
		if (contentType && contentType.includes("application/json")) {
			...
			if (text.length === 0 && response.status === 200) return Promise.resolve({ success: true } as unknown as T);
			try {
	...
	private static handleFetchError(error: unknown): void {
		if (error instanceof Error) NotificationManager.handleErrorCode(error.message);
		else NotificationManager.showError('An unknown error occurred');
	}
	...
	static async updateProfilePicture(imageData: string | File): Promise<any> {
		...
		try {
			...
			if (error instanceof TypeError && error.message.includes('null')) return { success: true };
			throw error;
		}
	}
	...
	static async getFriendList(userId: string): Promise<IReplyGetFriend[]> {
		...
		if (Array.isArray(friends)) {
			friends.forEach(friend => {
				if (friend.pic === 'default') friend.pic = '/images/default-avatar.svg';
			});
		}
		return friends;
	}
